{
  ...
  built = import ./builder.nix {
    inherit (pkgs-unstable) lib;
    bpkgs = pkgs-unstable;
    src0 = inputs.open-design;
  };
  daemon = built.daemon; # open-design-daemon (`od` CLI)
  web = built.web; # Next.js static SPA (apps/web/out)
