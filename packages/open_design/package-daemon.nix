...
# Vendored from upstream nexu-io/open-design's nix/package-daemon.nix (upstream
# retired its Nix distribution in #7644); `version` is now passed in from the
# builder instead of read from a sibling package.json.
#
# pnpm version note:
#   `package.json` declares `engines.pnpm` and pnpm enforces it on
#   `pnpm install` (regardless of `engine-strict`). The nixpkgs
#   default `pnpm` is generally incompatible. The builder overrides
#   `pnpm_10` to the exact tarball pinned by `packageManager`.
#
...
in
  stdenv.mkDerivation (finalAttrs: {
    ...
    # `fetchPnpmDeps` defaults to `pkgs.pnpm`; pin to the builder's
    ...
    buildPhase = ''
      ...
      #   better-sqlite3 only publishes prebuilds up to node-v131 (Node
      #   22). No v137 (Node 24) prebuild exists. `prebuild-install`
      #   would itself fail the GitHub fetch and fall through to a
      #   compile, so we skip the download attempt entirely and compile.
      #
      ...
      #   that on PATH gives us a `node-gyp` shim.
      ...
      # Build the filtered-in workspace packages in dependency order. `pnpm -r`
      # topologically sorts by the workspace graph, so this stays correct when
      # upstream reorganizes package dependencies across releases (a hand-sorted
      # path list does not — it silently builds a dependent before its dep).
